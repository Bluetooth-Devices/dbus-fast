...
def test_alias_resolves_to_base_and_signature(alias, base, code):
    resolved, sig = get_args(alias)
    assert resolved == base
    assert sig == DBusSignature(code)


def test_signature_carries_string():
    assert DBusSignature("(is)").signature == "(is)"


def test_signature_equal_by_value():
...
def test_custom_annotation_round_trips():
